...
namespace RazorPageClient.ServiceImplementions
{
    public class DepartmentService : IDepartmentService
    {
        ...
        {
            using HttpRequestMessage httpRequestMessage = new HttpRequestMessage(HttpMethod.Get, "/api/department/get-department-list");
        ...
        {
            using HttpRequestMessage httpRequestMessage = new HttpRequestMessage(HttpMethod.Get, $"api/department/get-department-select-list?selectedDepartment={selectedDepartment}");
            HttpResponseMessage response = await _httpClient.SendAsync(httpRequestMessage);
            if (response.IsSuccessStatusCode)
            {
                string repsonseString = await response.Content.ReadAsStringAsync();
        ...
        public async Task CreateDepartmentAsync(CreateDepartmentViewModel createDepartmentViewModel)
        {
            ...
            HttpResponseMessage response = await _httpClient.PostAsync("api/department/create-department", stringContent);
            if (!response.IsSuccessStatusCode)
            {
                throw new ApplicationException($"{response.ReasonPhrase}: The status code is: {(int)response.StatusCode}");
            }
        ...
        {
            HttpResponseMessage response = await _httpClient.GetAsync($"api/department/get-department/{departmentId}");
            if (response.IsSuccessStatusCode)
            {
        ...
        public async Task UpdateDepartmentAsync(UpdateDepartmentViewModel updateDepartmentViewModel)
        {
            ...
            HttpResponseMessage response = await _httpClient.PutAsync("api/department/update-department", stringContent);
            if (!response.IsSuccessStatusCode)
            {
                throw new ApplicationException($"{response.ReasonPhrase}: The status code is: {(int)response.StatusCode}");
            }
        ...
        {
            HttpResponseMessage response = await _httpClient.DeleteAsync($"api/department/delete-department/{departmentId}");
