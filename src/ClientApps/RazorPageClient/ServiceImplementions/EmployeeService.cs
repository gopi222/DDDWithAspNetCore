...
namespace RazorPageClient.ServiceImplementions
{
    public class EmployeeService : IEmployeeService
    {
        ...
        {
            HttpResponseMessage response = await _httpClient.GetAsync("api/employee/get-employee-list");
        ...
        {
            HttpResponseMessage response = await _httpClient.GetAsync($"api/employee/get-employee-details/{employeeId}");
            if (response.IsSuccessStatusCode)
            {
                string responseString = await response.Content.ReadAsStringAsync();
        ...
        public async Task CreateEmployeeAsync(CreateStudentViewModel createEmployeeViewModel)
        {
            ...
            HttpResponseMessage response = await _httpClient.PostAsync("api/employee/create-employee", content);
        ...
        public async Task UpdateEmplyeeAsync(UpdateStudentViewModel updateEmployeeViewModel)
        {
            string jsonStringBody = JsonSerializer.Serialize(updateEmployeeViewModel);
            using StringContent content = new StringContent(jsonStringBody, Encoding.UTF8, "application/json");
            HttpResponseMessage response = await _httpClient.PutAsync("api/employee/update-employee", content);
        ...
        {
            HttpResponseMessage response = await _httpClient.DeleteAsync($"api/employee/delete-employee/{employeeId}");
